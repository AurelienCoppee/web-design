                ...
                <div class="bg-surface-container p-6 rounded-lg shadow-mat-level3 w-full max-w-md m-4 relative">
                    ...
                    <Show when={props.mode() === "SETUP"}>
                        <form onSubmit={handleSetup2FASubmit} class="space-y-4">
                            <h2 class="text-xl font-semibold text-on-surface">Configurer l'Authentification à Deux Facteurs</h2>
                            <Show when={requestDetailsSubmission.pending}>
                                <p class="text-on-surface-variant">Chargement des détails 2FA...</p>
                            </Show>
                            <Show when={!requestDetailsSubmission.pending && (otpauthUrl() || qrCodeDataUrl())}>
                                ...
                                <Show when={!qrCodeDataUrl() && otpauthUrl()}>
                                    <p class="text-on-surface-variant text-sm">La génération du QR code a échoué, vous pouvez entrer cette clé manuellement :</p>
                                </Show>
                            ...
                            <button type="submit" disabled={verifyEnableSubmission.pending || requestDetailsSubmission.pending || !otpauthUrl()} class="w-full rounded-full bg-primary px-4 py-2.5 text-on-primary hover:brightness-110 disabled:opacity-50">
                    ...
                    <Show when={props.mode() === "LOGIN"}>
                        <form onSubmit={handleLoginWith2FASubmit} class="space-y-4">
                            <h2 class="text-xl font-semibold text-on-surface">Vérification à Deux Facteurs</h2>
                            <p class="text-on-surface-variant">Entrez le code de votre application d'authentification.</p>
                            <div>
                            ...
                            <button type="submit" class="w-full rounded-full bg-primary px-4 py-2.5 text-on-primary hover:brightness-110 disabled:opacity-50">
                                Se Connecter
                            </button>
                        </form>
                    </Show>
